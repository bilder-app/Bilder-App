...
import { getProductsByCategories } from "../api";
...
function CategoryFilter({ products, getProducts, route }) {
  ...
  useEffect(() => {
    getProductsByCategories([route.params]).then((resp) =>
      ...
      <View style={styles.content}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={{ marginLeft: 5 }}>
            <FontAwesomeIcon icon={faCaretDown} color="#E49012" size={25} />
          </TouchableOpacity>
        </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{ height: "100%", marginBottom: 10 }}>
              {productsData.map((data, index) => {
                return <ListItem product={data} key={index} />;
              })}
            </View>
          </ScrollView>
        </View>
