    ...
    <View style={{height: 500, width: 350}}>
      <ScrollContainer >
        {[1,2,3,4,5].map((item, index) => {
          return(
            <View style={{height: 120, width: '100%', backgroundColor: 'white', marginVertical: 10}} key={index}>
  ...
  .add("horizontal", () => (
    <View style={{height: 250, width: '100%', }}>
      <ScrollContainer position='horizontal'>
        {[1,2,3,4,5].map((item, index) => {
          return(
            <View style={{height: '100%', width: 120, backgroundColor: 'white', marginHorizontal: 10}} key={index}>
              <Text>Sample Text {item}</Text>
            </View>
          )
        })}
